...
if(isset($_POST['submit_sec_change']))
{
	...
	foreach($update_id as $updates_id)
	{
		...
		{
 			 mysql_query("update `student` set `roll_no`='$Studntroll_no',`section_id`='$Studntnew_sec' where `id` = '$updates_id'");
			 
 			 mysql_query("update `$sessios`.`stdnt_reg` set  `roll_no`='$Studntroll_no',`sec`='$Studntnew_sec' where `schlr_no` = '$Scholar_no'");
 		}
		$t++;
	}
}		
